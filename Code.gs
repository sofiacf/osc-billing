...
var billing = {
  ...
  formatSheet: function(sub, dets, charges, sheet){
    ...
    var rs = [s.getRange("A11:A14"), s.getRange(16,1, items.length, items[0].length),
        ...
        summary = [[today], dets.invNum, [period], [total]];
...
var format = billing,
    ...
    period = SpreadsheetApp.getActiveSpreadsheet().getSheets()[1].getName(),
    data = SpreadsheetApp.getActiveSpreadsheet().getSheets()[1].getDataRange().getValues();
...
function getDetails(subs){
  var details = new Array,
      sInfo = SpreadsheetApp.open(format.subDetailSheet).getSheets()[0].getDataRange().getValues();
  sInfo.shift();
  for (var i = 0; i<subs.length; i++) {
    ...
    if (!sInfo[i][sInfo[i].length-1]) {
      if (DriveApp.getFoldersByName(sInfo[i][0]).hasNext()){
        var r = SpreadsheetApp.open(format.subDetailSheet).getSheets()[0].getRange(i+2, sInfo[i].length);
...
function getSheet(sub, details, charges){
  ...
  const folder = DriveApp.getFoldersByName(sub).next(),
      template = format.template(sub), name = format.sheetName(sub, details);
